...
models = [(model_name, pipeline("zero-shot-classification", model=model_name, device=0 if device != 'cpu' else -1)) for model_name in model_names]
...
for filename in os.listdir(directory_path):
    if filename.endswith('.html'):
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            article_text = ' '.join([element.get_text(strip=True) for element in soup.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'])])
...
for filename, text in texts: 
    ...
    for model_name, model in models:
        data[model_name] = {}
        for keyphrase in keyphrases: #iterate over each one instead of all at once to get independent scores
